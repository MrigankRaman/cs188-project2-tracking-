...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in range(len(livingGhostPositionDistributions)):
            pos[i] = livingGhostPositionDistributions[i].argMax()
        min = float('inf')
        bestaction = legal[0]
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            for i in range(len(livingGhostPositionDistributions)):
                if min > self.distancer.getDistance(pos[i],successorPosition ):
                    min = self.distancer.getDistance(pos[i],successorPosition )
                    bestaction = action
        #print(len(livingGhosts))
